...
import app/domain/auth_session/sql as auth_session
...
pub type VerifyAuthSessionTokenError {
  InvalidToken
  ExpiredOrNotFound
  DatabaseFailure(QueryError)
}

pub fn verify(token: AuthSessionToken, ctx: Ctx) {
  use session <- result.try(
    auth_session.select_auth_session_by_id(ctx.db, token.id)
