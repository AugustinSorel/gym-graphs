...
export const sendTeamJoinRequestEmail = async (
  toAddresses: Array<TeamInvitation["email"]>,
  team: Team,
) => {
  const config = emailConfig(
    toAddresses,
    `Invitation to join ${team.name}`,
    emailTemplates.joinRequest(team),
  );
...
const emailTemplates = {
  joinRequest: (team: Team) => {
    const url = `${env.APP_URL}/teams/${team.id}/settings`;
    ...
    return `
      ...
      <html lang="en">
        <head>
          <meta charset="UTF-8" />
          <title>New Join Request Received</title>
          <style>
            * {
              margin: 0;
              padding: 0;
        ...
        <body class='text-foreground p-4'>
          <div class="mx-auto max-w text-center">
            <h1 class='text-2xl text-primary'>New Join Request</h1>
            <p class='text-muted-foreground text-sm mt-1'>
              A new join request has been submitted for the team ${team.name}.
            </p>
            <p class='text-muted-foreground text-sm'>
              Click the link below to review and manage the request.
            </p>
            <a
              href="${url}"
              ...
              target="_blank"
            >
              <strong class='text-accent-foreground'>Review Request</strong>
            </a>
            <p class='text-muted-foreground text-xs'>
              This request will remain active until you review it. 
              <br />
