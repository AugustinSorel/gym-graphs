...
const renderStatsVolumeGraph = (container, points, unit) => {
  ...
  const slots = d3.range(STATS_DAYS).map((i) => {
    const offset = i - (STATS_DAYS - 1); // -6 … 0
    const date = new Date();
    date.setDate(date.getDate() + offset);
    return {
      slotIndex: i,
      dayOffset: offset,
      label: `${STATS_MONTHS[date.getMonth()]} ${date.getDate()}`,
      isToday: offset === 0,
  ...
  const svg = d3
    ...
    .attr("aria-label", "Weekly volume — total across all exercises, last 7 days")
    .attr("role", "img");
  ...
  // Tooltip
  let tooltip = container.querySelector(".stats-vol-tooltip");
  if (!tooltip) {
    tooltip = document.createElement("div");
    tooltip.className = "stats-vol-tooltip";
  ...
  slotG
    .filter((d) => d.slotIndex === 0 || d.slotIndex === STATS_DAYS - 1)
    .append("text")
    .attr("x", slotW / 2)
    .attr("y", innerH + 16)
...
const getStatsVolumeData = () => {
  try {
    const el = document.getElementById("stats-volume-data");
    if (!el) return [];
...
let statsVolumeResizeObserver = null;

function initStatsVolumeGraph() {
  const container = document.getElementById("stats-volume-graph");
  if (!container) return;

  const points = getStatsVolumeData();
  const unit = container.dataset.unit || "kg";

  renderStatsVolumeGraph(container, points, unit);

  if (statsVolumeResizeObserver) {
    statsVolumeResizeObserver.disconnect();
  }
  statsVolumeResizeObserver = new ResizeObserver(() => {
    renderStatsVolumeGraph(container, points, unit);
  });
  statsVolumeResizeObserver.observe(document.documentElement);
}

document.addEventListener("DOMContentLoaded", initStatsVolumeGraph);
